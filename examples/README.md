...
|---:|---|---|
| 1 | [`server.cpp`](server.cpp) + [`client.cpp`](client.cpp) | one-shot fixed RPC and streaming |
| 2 | [Getting started](../docs/getting-started/README.md) | build, run, and choose an API path |
| 3 | [Robot HAL server](robot_hal_server.cpp) | real resource mutexes paired with named domains, queue policies, context, and stats |

The basic example intentionally uses plain `on()` and `Client::request()`. The
Robot HAL example is the advanced composition: its drivetrain and arm mutexes
