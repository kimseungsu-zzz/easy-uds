...
void write_request(int fd, std::string_view route, std::string_view body, std::size_t max_message_size,
    ...
    write_header(fd, WireType::request, static_cast<std::uint32_t>(route.size()), static_cast<std::uint32_t>(body.size()),
                 inactivity_timeout, absolute_deadline);
    write_exact(fd, route.data(), route.size(), inactivity_timeout, absolute_deadline);
    write_exact(fd, body.data(), body.size(), inactivity_timeout, absolute_deadline);
}

Request read_request(int fd, std::size_t max_message_size, std::chrono::milliseconds inactivity_timeout,
                     Deadline absolute_deadline) {
    const auto header = read_header(fd, WireType::request, inactivity_timeout, absolute_deadline);
...
void write_response(int fd, const Response& response, std::size_t max_message_size,
    ...
    write_header(fd, WireType::response, static_cast<std::uint32_t>(response.status_code),
                 static_cast<std::uint32_t>(response.body.size()), inactivity_timeout, absolute_deadline);
    write_exact(fd, response.body.data(), response.body.size(), inactivity_timeout, absolute_deadline);
}
...
void handle_client(const std::shared_ptr<detail::ServerState>& state, int fd, Deadline deadline) noexcept {
    try {
        const Request request = read_request(fd, state->options.max_message_size, state->options.io_timeout, deadline);
...
void Server::run() {
    ...
    while (state->running.load()) {
        ...
        if (client_fd < 0) {
            ...
            accept_error = std::make_exception_ptr(std::system_error(error, std::generic_category(), "accept failed"));
            break;
        }
